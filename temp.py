...
if __name__ == "__main__":
    img = cv2.imread("./dataset/cover.jpg")
    wm = cv2.imread("./dataset/secret2.png", cv2.IMREAD_GRAYSCALE)
    dct = DCT_Watermark()
    wmd = dct.embed(img, wm)
    cv2.imwrite("./results/watermarked.jpg", wmd)

    img = cv2.imread("./results/watermarked.jpg")
    dct = DCT_Watermark()
    signature = dct.extract(img)
    cv2.imwrite("./results/signature.jpg", signature)